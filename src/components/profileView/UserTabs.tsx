  ...
  useEffect(() => {
    const fetchCommentsCounts = async () => {
      try {
        ...
        await Promise.all(
          userIdeas.map(async (idea) => {
            try {
              ...
              const commentsCount = Array.isArray(data)
                ? data.length
                : data.comments?.length || 0;
              counts[idea.id] = commentsCount;
            } catch (err) {
        ...
        console.error("Failed to calculate comment counts:", err);
      }
    };
        ...
        <button
          ...
          className={`flex items-center gap-2 cursor-pointer transition ${
            activeTab === "challenges"
              ? "bg-bg py-1 px-2 rounded text-text-primary"
              : "text-text-secondary px-2 py-1 hover:bg-bg-gray hover:text-text-secondary"
          }`}
        >
              ...
              <Link href={`/idea/${idea.id}`}>
                ...
                <div className="flex gap-4 text-xs mt-2 text-text-secondary">
                  ...
                  <span className="flex gap-1 items-center">
                    ...
                    {commentCounts[idea.id] ?? 0} comments
                  </span>
                </div>
              </Link>
            </div>
